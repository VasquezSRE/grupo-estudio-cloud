...
bucket_name = "lab3-sports-analytics-data-lake"  # Change to a unique S3 bucket name
...
def execute_athena_query(query_string, database_name, output_location):
    """Executes an Athena query and waits for its completion, then prints results."""
    try:
        ...
        if status == "SUCCEEDED":
            print(f"Query SUCCEEDED. Results are in {output_location}{query_execution_id}.csv")
            result_key = f"{query_execution_id}.csv"
            try:
                obj = s3_client.get_object(Bucket=bucket_name, Key=f"athena-results/{result_key}")
                results_content = obj['Body'].read().decode('utf-8')
                print("\n--- Athena Query Results ---")
                print(results_content)
                print("----------------------------\n")
            except Exception as e:
                print(f"Error reading results from S3: {e}")

        else:
            reason = query_status["QueryExecution"]["Status"].get("StateChangeReason", "N/A")
            print(f"Query {status}: {reason}")
            error_message = query_status["QueryExecution"]["Status"].get("AthenaError", {}).get("ErrorMessage")
            if error_message:
...
# Main workflow (modificado para incluir la consulta)
def main():
    ...
    if nba_data:  # Only proceed if data was fetched successfully
        ...
        create_glue_table()
        configure_athena() # This just sets the output location

        print("\nRunning a sample Athena query...")
        sample_query = f"SELECT PlayerID, FirstName, LastName, Team, Position, Points FROM nba_players LIMIT 5;"
        execute_athena_query(sample_query, glue_database_name, athena_output_location)
